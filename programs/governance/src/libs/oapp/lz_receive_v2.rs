use oapp::{endpoint_cpi::EVENT_SEED};
use anchor_lang::{
    prelude::*,
    solana_program::{
...
use oapp::endpoint::{NONCE_SEED, OAPP_SEED, PAYLOAD_HASH_SEED, ENDPOINT_SEED, COMPOSED_MESSAGE_HASH_SEED};
...
pub fn deserialize_alt(alt: &AccountInfo) -> Result<Vec<Pubkey>> {
    AddressLookupTable::deserialize(*alt.try_borrow_data().unwrap())
        ...
        .map_err(|_e| error!(ErrorCode::InvalidAddressLookupTable))
}
