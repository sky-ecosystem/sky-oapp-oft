...
task('lz:oft:solana:setPauser', 'Set the pauser for the OFT program')
    .addParam('pauser', 'The pauser address', undefined, devtoolsTypes.string)
    .addParam('fromEid', 'The source endpoint ID', undefined, devtoolsTypes.eid)
    .setAction(async (args: Args) => {
        const { pauser, fromEid, computeUnitPriceScaleFactor } = args
        ...
        console.log(`✅ Set pauser for OFT program to ${pauser}!`)
