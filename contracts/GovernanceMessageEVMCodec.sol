...
library GovernanceMessageEVMCodec {
    ...
    uint8 private constant GOVERNANCE_CONTRACT_OFFSET = DST_EID_OFFSET + 4;
    uint8 private constant GOVERNED_CONTRACT_OFFSET = GOVERNANCE_CONTRACT_OFFSET + 20;
    ...
    function decode(bytes calldata _msg) internal pure returns (GovernanceMessage memory message) {
        ...
        message.dstEid = uint32(bytes4(_msg[DST_EID_OFFSET:GOVERNANCE_CONTRACT_OFFSET]));
        message.governanceContract = address(uint160(bytes20(_msg[GOVERNANCE_CONTRACT_OFFSET:GOVERNED_CONTRACT_OFFSET])));
